...
extract($_POST);
$date = $date.'-01';
$chk = $conn->query("SELECT * from billing where tenant_id = $id and date_format(billing_date,'%Y-%m') = '".date("Y-m",strtotime($date))."'")->num_rows;
if($chk > 0 && !isset($bid)){
	echo "<div class='container-fluid'><center><h3><b>Tenant's Bill already exist for the selected Month.</b></h3></center></div>";
	exit;
}
$query = $conn->query("SELECT * FROM tenants where id=".$id)->fetch_array();
foreach($query as $k => $v){
	$$k = $v;
}
$monthly_rent = 0;
$blk = '';
$block = $conn->query("SELECT * FROM block_locations where id in ($block_ids) ");
while($row = $block->fetch_array()){
	$monthly_rent += intval($row['rate']);
	if(empty($blk))
			$blk .= "Blk. ".$row['block']." ".$row['floor']." Floor";
		else
			$blk .= ", Blk. ".$row['block']." ".$row['floor']." Floor";
}
$billing = $conn->query("SELECT * from billing where tenant_id = $id and date(billing_date) < '".date("Y-m-d",strtotime($date))."' order by date(billing_date) desc limit 1");
if($billing->num_rows > 0){
	$result =$billing->fetch_array(); 
$pdate = $result['billing_date'];
$previous = $conn->query("SELECT * from bills where billing_id = ".$result['id']);
...
if(isset($bid)){
$erate = $conn->query("SELECT * from bills where billing_id = '".$bid."' and type = 2 ");
if($erate->num_rows > 0){
	$erate_res = $erate->fetch_array();
	$sys_e_rate = $erate_res['rate'];
	$econs = $erate_res['consumption'];
}
$wrate = $conn->query("SELECT * from bills where billing_id = '".$bid."' and type = 3 ");
if($wrate->num_rows > 0){
...
<div class="container-fluid">
	<div class="row">
		<div class="col-lg-5" id="prev_det">
			<p>Rented Block/s: <b class="float-right"><?php echo $blk ?></b></p>
		...
		<div class="col-lg-7">
			<table class="table table-condensed clac-tbl">
				...
				<tr class="payment">
					...
					<th class="text-right"><input type="number" class="form-control text-right" step="any" value="0" name="amount_tendered"></th>
				</tr>
				<tr class="payment">
					<th>Change</th>
...
<script>
	...
	$('[name="amount_tendered"]').on('keyup keydown keypress changed',function(){	
	...
	function pay(){
		if($('[name="amount_tendered"]').val() <= 0 || $('[name="amount_tendered"]').val() == '' ){
			alert_toast("Amount tendered must be greater than or equal to the amount due","warning")
			return false;
		}
		start_load()
		$.ajax({
			...
			method:'POST',
			data:{id:<?php echo isset($bid) && $bid > 0 ? $bid :0 ?>,amount_tendered:$('[name="amount_tendered"]').val(),amount_change:$('[name="amount_change"]').val()},
