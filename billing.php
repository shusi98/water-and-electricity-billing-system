					...
					<div class="card-body">
						...
						<table class="table table-bordered table-condensed table-hover">
							...
							<tbody>
								...
								$billing = $conn->query("SELECT b.*,t.name,t.owner from billing b inner join tenants t on t.id = b.tenant_id where date_format(b.billing_date,'%Y-%m') = '$month' order by b.id asc");
									while($row=$billing->fetch_assoc()):
									$chk =  $conn->query("SELECT b.*,t.name,t.owner from billing b inner join tenants t on t.id = b.tenant_id where date(b.billing_date) > '".$month."-01' and b.id != '".$row['id']."' and b.tenant_id = '".$row['tenant_id']."' order by date(b.billing_date) asc")->num_rows;
								?>
								<tr>
									...
									<td class="">
										 <p> Business Name: <b><?php echo ucwords($row['name']) ?></b></p>
										 <p> Owner Name: <b><?php echo ucwords($row['owner']) ?></b></p>
									</td>
									<td class="">
...
<script>
	...
	$('#new_billing').click(function(){
		uni_modal("New Tenant's Billing","manage_billing.php","large")
	})
	
	$('.edit_billing').click(function(){
		uni_modal("Edit Tenant's Billing","manage_billing.php?id="+$(this).attr('data-id'),"large")
		
	})
	$('.view_billing').click(function(){
		uni_modal("Tenant's Billing","view_bill.php?id="+$(this).attr('data-id'),"large")
		
	})
	$('.delete_billing').click(function(){
		_conf("Are you sure to delete this Tenants Bill?","delete_billing",[$(this).attr('data-id')])
	})
